...
async function GetData(pageNum = 1) {
    ...
    const post = await fetch(`/Dados/GetGruposPaged/`, requestOptions);
...
async function remove(uuid, descricao) {
    if (confirm(`Deseja remover o cadastro ${descricao}? Essa ação não poderá ser desfeita!`) === true) {
        await fetch(`/Dados/RemoverGrupo?uuid=${uuid}`, {method: "GET"});
...
async function gravarGrupo() {
    ...
    const post = await fetch("/Dados/PostGrupo/", requestOptions);
    //const response = post.text();
